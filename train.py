...
class TrainingUnit:
    def __init__(self, input_metrics, target_metrics, target_metrics_thresholds, max_number_of_scaling_activity=100, training_samples_required=10,nn_stop_error_rate=10.0, max_delta_vm=2):
        ...
        self.training_samples_required = training_samples_required
    ...
    def train(self, sample_number):
        if sample_number < self.training_samples_required:
            print('Cannot start training yet')
            #return something that indicates we cannot learn now and advice should do the work, the inspection 
            return [None]*4 
        else: 
            ...
            target_metric_min = self.target_metrics_thresholds[0].get('min_threshold')
            target_metric_max = self.target_metrics_thresholds[0].get('max_threshold')
